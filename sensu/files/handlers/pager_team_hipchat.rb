#!/usr/bin/env ruby
#
# Sensu Handler: pager_team_hipchat (MediaMath fork of 'hipchat' community handler)
#
# This handler script is used to send notifications to Hipchat rooms based on event[:check][:pager_team] value.
#
# Input:
...
class HipChatNotif < Sensu::Handler
  ...
  def handle
    json_config = if ! @event['check']['pager_team'].nil?
                    @event['check']['pager_team'] + '_hipchat'
                  else
                    config[:json_config] || 'hipchat'
                  end
